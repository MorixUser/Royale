(function() {
	...
	function registerCtrl($rootScope, $state, $scope, $translate, localStorageService, $http, msgDialogSrvc,$filter) {
		...
		$scope.years=[];
		for(var i=0;i<100;i++){
			var y = $scope.myDate.getFullYear() - 18-i;
			$scope.years.push({name:y, value:y});
		}
		$scope.months=[];
		for(var i=0;i<12;i++){
			var y = i+1;
			$scope.months.push({name:y, value:y});
		}
		$scope.days=[];
		for(var i=0;i<31;i++){
			var y = i+1;
			$scope.days.push({name:y, value:y});
		}
		...
		var code = getParameterByName('bc');
		if(code){
			if(!$scope.user)
				$scope.user={};
			$scope.user.promotioncode=code;
		}
		...
		$scope.getRegisterUser = function(user) {
			...
			switch(user.bm){
				case 2:
					if(user.bd>29)
						msgDialogSrvc.showDialog('#registerForm', 'error', "invalid_birthdate");
						return;
					break;
				case 4:
					if(user.bd>30)
			...
			var bdate = user.by+"-"+user.bm+"-"+user.bd; //moment(user.bdate).format('YYYY-MM-DD');
			...
			var url = apiURL + "/user/getsignup?username="+user.username+"&hash1="+hash1+"&salt="+salt+"&hash2="+hash2+"&questionid="+user.persq+"&answer="+user.persa+"&agbok="+user.agreement+"&birthday="+bdate+"&countrycode="+user.country+"&currencycode="+user.currency+"&email="+user.mail+"&firstname="+user.firstName+"&lastname="+user.lastName+"&mobilenumber="+user.mobile+
			"&place=" + user.place + "&postalcode=" + user.postalCode + "&promotioncode=" + user.promotioncode + "&street=" + user.street + "&title=&gender=" + user.gender + "&gendershort=" + gendershort + "&nationality=" + user.nation + "&passport=" + user.passport;
