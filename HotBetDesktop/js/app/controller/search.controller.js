(function () {
    ...
    function searchCtrl($rootScope, $state, $scope, $translate, localStorageService, $http, $stateParams, $filter, loginData, betSlipSrvc, $timeout, $q, $window) {
        ...
        self.searchGame = function (searchText) {
            return $http.get(apiURL + "/bet/get?pagesize=50&name=" + searchText)
        ...
        self.selectedItemChange = function (item) {
            ...
            console.log($scope.searchButtonDisabled);
        };

        self.getGameTextForAutoComplete = function (item) {
            return item.t1n + " - " + item.t2n;
        };

        self.finalizeSearch = function (item) {
            $scope.loadSpecial(item);
            localStorageService.set('searchSelectedItem', $scope.selectedItem);
            if ($state.current.name != 'sportbets') {
                localStorageService.set('searchGameInfo', { gameId: item.gid, mId: item.mid, gs: item.gs });
                $state.go("sportbets", { });
            } else {
                $rootScope.$broadcast('SEARCH_OK_EVENT', { gameId: item.gid, mId: item.mid, gs: item.gs });
            }
          
            //$state.go("specialbets", { gameId: item.gid, mId: item.mid, gs: item.gs });
            self.selectedSearchItem = '';
            self.searchText = '';
            $scope.searchButtonDisabled = true;
            $rootScope.$broadcast('CLOSE_SEARCHBOX_EVENT', {});
        };
