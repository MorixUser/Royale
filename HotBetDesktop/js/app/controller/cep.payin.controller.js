(function() {
	...
	angular
	.module('betApp')
	.controller('cepbankpayinCtrl', cepbankpayinCtrl);

	function cepbankpayinCtrl($rootScope, $state, $scope, $translate, localStorageService, $http, $stateParams, loginData, $filter, msgDialogSrvc, $mdMedia) {
		...
		$scope.requestPayment = function (user) {
		    ...
		    var url = apiURL + "payment/GetPerformCepDepositMobile?value="+user.amount+"&personalid="+user.personalid+
                "&bankname="+user.bank+"&name="+user.name+"&pin="+user.pin+"&PhoneNumber1="+user.phonenumber1+
                "&PhoneNumber2=" + user.phonenumber2 + "&birthdate=" + bdate;
			$http
