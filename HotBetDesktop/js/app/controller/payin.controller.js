(function () {
    ...
    function payinCtrl($rootScope, $state, $scope, $translate, localStorageService, $http, $stateParams, loginData, $filter, msgDialogSrvc, $mdMedia, $document) {
        $scope.payinType = $stateParams.ptype ? $stateParams.ptype : 'Skrill';
        ...
        $scope.checkoutapco = function () {
			...
			.success(function (data) {
			    ...
			    if (data.success) {
			        ...
			        $http
                   .get(apiURL + "/payment/GetCheckoutPage?payout=false&amount=" +$scope.amount + "&type=" +$scope.selectedPayMethod + "&currency=" +$scope.selcurrency+"&upin=&apcardnumber=&apcvv=0&apyear=&apmonth=",
