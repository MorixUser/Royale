(function () {
	...
	function liveGamesCtrl($rootScope, $state, $scope, $translate, localStorageService, $http, $stateParams, $filter, loginData, betSlipSrvc, $timeout, msgDialogSrvc, $q, $window) {
		...
		$scope.allsports = [
            { id: -1, name: 'favorites', count: 0, main: $scope.usefavorites },
            { id: 0, name: 'all', count: 0, main: true },
            { id: 1, name: 'soccer', count: 0, main: true },
            { id: 2, name: 'basketball', count: 0, main: true },
            { id: 3, name: 'baseball', count: 0, main: (!$scope.usefavorites) },
            { id: 4, name: 'icehockey', count: 0, main: true },
            { id: 5, name: 'tennis', count: 0, main: true },
            { id: 6, name: 'handball', count: 0 },
            { id: 12, name: 'rugby', count: 0 },
            { id: 16, name: 'football', count: 0 },
            { id: 19, name: 'snooker', count: 0 },
            { id: 20, name: 'tabletennis', count: 0 },
            { id: 21, name: 'cricket', count: 0 },
            { id: 22, name: 'darts', count: 0 },
            { id: 23, name: 'volleyball', count: 0 },
            { id: 29, name: 'futsal', count: 0 },
            { id: 31, name: 'badminton', count: 0 },
            { id: 34, name: 'beach_volleyball', count: 0 },
		];
		$scope.sportvis = [];
		$scope.allsportsCount = function () {
		    var sum = 0;
		    angular.forEach($scope.allsports, function (item, v) {
		        sum+=item.count;
		    });
		    return sum;
		}

		$scope.mainsports = function () {
		    return $scope.allsports.filter(function (x) { return x.main });
		}
		$scope.secondarysports = function () {
		    return $scope.allsports.filter(function (x) { return !x.main });
		}
		...
		$scope.selectBetType = function (type) {
			...
			if ($scope.multibetSelection) {
				$scope.currentBetTypes.filter(function (x) {
					return x.name == type.name
				})[0].selected = !$scope.currentBetTypes.filter(function (x) {
					return x.name == type.name
				})[0].selected;
			} else {
		...
		$scope.getmainbettypes = function () {
			if ($scope.netsports.indexOf($scope.sportId) > -1)
				return $scope.currentBetTypes.filter(function (x) {
					return x.main && x.netsport
				});
			if ($scope.othersports.indexOf($scope.sportId) > -1)
				return $scope.currentBetTypes.filter(function (x) {
					return x.main && x.othersport
				});

			return $scope.currentBetTypes.filter(function (x) {
				return x.main && !x.onlynetsport && !x.onlyothersport
			});
		}
		$scope.getmorebettypes = function () {
			return $scope.currentBetTypes.filter(function (x) {
				return $scope.netsports.indexOf($scope.sportId) <= -1 && $scope.othersports.indexOf($scope.sportId) <= -1 && (!x.main || ($scope.sportId <= 0 && (x.netsport || x.othersport)))
			});
		}
		$scope.getselectedbettypes = function (item) {

		    if(item){
		        return $scope.currentBetTypes.filter(function (x) {
		            return x.selected == true && (x.gt==1 || (x.gt==2 && item.gs=='FirstPeriod' && item.sid==1))
		            });
		    }
		...
		$scope.favClick = function (item) {
			...
			if ($scope.favgames.filter(function (x) {
					return x == item.gn
				})[0]) {
				$scope.favgames = $scope.favgames.filter(function (x) {
					return x != item.gn
				});
				item.fav = false;
			} else {
			...
			$scope.allsports[0].count = $scope.favgames.length;
		}
		...
		$scope.getSports = function () {
			...
			$http
				...
				.success(function (data) {
				    ...
				    angular.forEach($scope.sports, function (item, value) {
				        var ms = $scope.allsports.filter(function (x) { return x.id == item.id; })[0];
				        if (ms)
				            ms.count = item.cntgames;
				        item.show = true;
				    })
				    $scope.allsports[1].count = $scope.allsportsCount();
		...
		$scope.getsportname = function (sid) {
			var s = $scope.sports.filter(function (x) {
				return x.id == sid
			})[0];
			if (s)
				return s.name;
		...
		$scope.gettournamentname = function (tid) {
			var s = $scope.items.filter(function (x) {
				return x.tid == tid
			})[0];
			if (s)
				return s.cn+" / "+s.tn;
		...
		$scope.loadTodayGames = function () {
		    $scope.showToday = !$scope.showToday;
		    //angular.forEach($scope.todaygames(), function (item, value) {
		    //    item.hidden = !$scope.showToday;
		    //});
		}
		...
		$scope.tomorrowGamesLoaded = false;
		$scope.loadTomorrowGames = function () {
		    if (!$scope.tomorrowGamesLoaded) {
		        $scope.showTomorrow = true;
		        $scope.tomorrowGamesLoaded = true;
		        $scope.getGames(moment().add('days', 1).format('YYYY-MM-DD'), true);
		    }
		    else {
		        $scope.showTomorrow = !$scope.showTomorrow;
		        //angular.forEach($scope.tomorrowgames(), function (item, value) {
		        //    item.hidden = !$scope.showTomorrow;
		        //});
		        
		    }
		}
		$scope.getGames = function (fordate,addgames) {
					...
					$http.get(url)
						.success(function (data) {
							...
							angular.forEach(data.items, function (i, v) {
								...
								var gametips = data.tips.filter(function (x) {
									return x.gid == i.gid
								});
			...
			var url = apiURL + "/live/get?pagesize=10&pagenumber=1" + '&lang=' + $rootScope.currentLang + "&usepaging=false&StartDate="+fordate;
			...
			$http
				...
				.success(function (data) {
				    if (!addgames)
					    $scope.items = [];
				    console.log('games: ' + data.items.length);
		...
		$scope.livegames = function () {
		    var list = $scope.items.filter(function (x) { return x.gs != 'NotStarted' && x.sid == 1 });

		    return $filter('orderBy')(list, 'sorting');

		}
		$scope.livegamesothersports = function () {
		    var list= $scope.items.filter(function (x) { return x.gs != 'NotStarted' && x.sid != 1 });
		    return $filter('orderBy')(list, 'sorting');
		}
		$scope.livegameall = function () {
		    var list= $scope.items.filter(function (x) { return x.gs != 'NotStarted'});
		    return $filter('orderBy')(list, 'sorting');
		}
		$scope.todaygames = function () {   
		    var list= $scope.items.filter(function (x) { return x.gs == 'NotStarted' && moment(x.gd) < moment().endOf('day') && (x.sid == $scope.sportId || $scope.sportId == 0) });
		    return $filter('orderBy')(list, 'sorting');
		}
		$scope.tomorrowgames = function () {
		    var list = $scope.items.filter(function (x) { return x.gs == 'NotStarted' && moment(x.gd) >= moment().endOf('day') && (x.sid == $scope.sportId || $scope.sportId == 0) });
		    return $filter('orderBy')(list, 'sorting');
		}
		...
		$scope.loadSpecial = function (item) {
		    ...
		    angular.forEach($scope.items, function (match, value) {
		        if (match.gid != item.gid)
		            match.showSpecialbets = false;
		    });

		    $rootScope.gameInfo = item;
		    item.showSpecialbets = !item.showSpecialbets;
		    //ui-sref="specialbets({gameId: item.gid, mId: item.mid, gs: item.gs})"
			try {
		...
		$scope.addTip = function (gameid, tipid, live) {
			if ($scope.tipq.filter(function (x) {
					return x.tipid == tipid
				})[0])
				return;
			$scope.tipq.push({
				gameid: gameid,
		...
		$scope.addTipBase = function (gameid, tipid, live) {
			...
			$http.get(url, loginData.getAuthOptions())
				.success(function (data) {
					...
					$rootScope.$broadcast('ADDTIP_EVENT', { data: data });

					if ($scope.selectedTips.indexOf(tipid)>=0)
					    $rootScope.$broadcast('ADDMAINTIP_EVENT', { tipid: tipid });
					else
					    $rootScope.$broadcast('REMOVETIP_EVENT', { tipid: tipid });
					deferred.resolve();
				})
				.error(function (data) {
		...
		$scope.isTipSelected = function (name, tipid) {
			var tipexists = $scope.selectedTips.filter(function (x) {
				return x == tipid
			})[0];
		...
		$scope.addNew = function (item, init) {
			...
			angular.forEach(item.tips, function (tip, v) {
				switch (tip.tp) {
					...
					case "5":
						...
						break;
					case "6":
						tip.bt = $scope.netsports.indexOf(item.sid) > -1 ? -1 : 3;
						tip.gt = 1;
						tip.sn = "x";
						break;
					case "7":
					...
					case "8":
						...
						break;
					case "9":
						tip.bt = $scope.netsports.indexOf(item.sid) > -1 || $scope.othersports.indexOf(item.sid) > -1 ? -1 : 8;
						tip.gt = 1;
						tip.sn = "x";
						break;
					case "10":
			...
			var i = $scope.items.filter(function (x) {
				return x.gid == item.gid
			})[0];
			if (!i) {
				item.setscores = [];
		...
		$scope.setGameData = function (item, i) {
			...
			$scope.setSorting(i, ogs);
		};

		$scope.gameindex = 0;
		$scope.setSorting = function (i, ogs) {
		    var sortcat = 'z';
		    var sortmin = 500;
		    $scope.gameindex++;
		    var sortnr = $scope.gameindex;
		    ...
		    switch (i.gs) {
		        ...
		        case 'SecondHalfOverTime':
		            sortcat = 'n';
		            sortmin = 500 - i.mp;
		            break;

		        case 'HalfTimeOverTime':
		            sortcat = 'o';
		            break;

		        case 'FirstHalfOverTime':
		            sortcat = 'p';
		            sortmin = 500 - i.mp;
		            break;
		        ...
		        case 'SecondPeriod':
		            sortcat = 'w';
		            if (i.mp.indexOf("90+") > -1)
		                sortmin = 500 - 90;
		            else
		                if (i.mp.indexOf("46+") > -1)
		                    sortmin = 500 - 46;
		                else
		                    if (i.mp.indexOf("47+") > -1)
		                        sortmin = 500 - 47;
		                    else
		                        sortmin = 500 - i.mp;
		            break;
		    ...
		    vsorting = sortcat + sortmin + sortnr;

		    if ((!i.sorting || i.gs != ogs))
		        i.sorting = vsorting;
		};
		...
		$scope.handleGameRemove = function (i) {
			$scope.items = $scope.items.filter(function (x) {
				return x.gid != i.gid
			});
		};
		...
		$scope.setMainOdds = function (tip, i, newgame) {
			...
			if (tip.tp == '3' || tip.tp == '4' || tip.tp == '17' || tip.tp == '18') {
				...
				if (tip.tp == '3' || tip.tp == '4') {
				    if (!(tip.ac == false && i.ouvalue != tip.vl))
				        $scope.setMainTipProperties(tip, i.tips[parseInt(tip.tp)], newgame);
				}
				if (tip.tp == '17' || tip.tp == '18') {
				    if (!(tip.ac == false && i.ouvalueht != tip.vl))
				        $scope.setMainTipProperties(tip, i.tips[parseInt(tip.tp)], newgame);
				}
			} else {
				if (tip.ac || cs == tip.vl.trim()|| tip.vl=='' || tip.mb)
					$scope.setMainTipProperties(tip, i.tips[parseInt(tip.tp)], newgame);
			}
		};

		$scope.staticsUrl = function (item) {
		    var url = "https://livescore.betradar.com/s4/?clientid=850&fragment=season&matchid=" + item.mid + "&language=" + $rootScope.currentLang;
		    return "https://href.li/?" + url;
		};
		$scope.openStatistics = function (item) {
		    var url = $scope.staticsUrl(item);
		    showStatDialog(url, item.mid);
		};
		...
		angular.element(document).ready(function () {
			...
			var betSlip = betSlipSrvc.init().then(function (data) {
				...
				$scope.getGames(moment().format('YYYY-MM-DD'));
			});
		});
		...
		var deregGameChangeEvent = $rootScope.$on('GAMECHANGE_EVENT', function (event, args) {
			...
			if (moment(item.gd) > moment().add('days', 1).endOf('day'))
			    return;
			...
			if (!i) {
				...
				i = $scope.items.filter(function (x) {
					return x.gid == item.gid
				})[0];
			}
		...
		var deregAddTipEvent = $rootScope.$on('ADDSPECIALTIP_EVENT', function (event, args) {
		    $scope.selectedTips.push(args.tipid);
		});
		var deregRemoveTipEvent = $rootScope.$on('REMOVETIP_EVENT', function (event, args) {
		    $scope.selectedTips = $scope.selectedTips.filter(function (x) {return x != args.tipid});
		});
		var deregClearBetSlipEvent = $rootScope.$on('CLEARBETSLIP_EVENT', function (event, args) {
		    $scope.selectedTips = [];
		});
