(function() {
	...
	function registerCtrl($rootScope, $state, $scope, $translate, localStorageService, $http, msgDialogSrvc,$filter) {
		$rootScope.currentPage = $filter('translate')('REGISTER');
		...
		$scope.openTermsAndConditions = function () {
		    var url = 'info/{lang}/agb/index.html';
		    url = url.replace('{lang}', $rootScope.currentLang);
		    newWindow = window.open(url, '', 'scrollbars=no,location=no, titlebar=no,menubar=no');
		}
		
		$scope.getRegisterUser = function (user) {

		    if (!user.password || user.password.length < 6) {
		        msgDialogSrvc.showDialog('#wrap', 'error', 'min_password_length');
		        return;
		    }

		    if (user.password != user.password2) {
		        msgDialogSrvc.showDialog('#wrap', 'error', 'passwords_are_not_equal');
		        return;
		    }

			var salt = NewSalt();
			var bdate = moment(user.bdate).format('YYYY-MM-DD');
			var hash1 = CryptoJS.SHA256(salt+user.password);
			var hash2 = CryptoJS.SHA256(salt+user.password2);
			var gendershort = 0;
			if(!user.gender)
				user.gender="male";
			if (user.gender == "male") {
			...
			var url = apiURL + "/user/getsignup?username=" + user.username + "&hash1=" + hash1 + "&salt=" + salt + "&hash2=" + hash2 + "&questionid=" + user.persq + "&answer=" + user.persa + "&agbok=" +
                user.agreement + "&birthday=" + bdate + "&countrycode=" + user.country + "&currencycode=" + user.currency + "&email=" + user.mail + "&firstname=" + user.firstName +
                "&lastname=" + user.lastName + "&mobilenumber=" + user.mobile +
			"&place=" + user.place + "&postalcode=" + user.postalCode + "&promotioncode=" + user.promotioncode + "&street=" + user.street +
            "&title=&gender=" + user.gender + "&gendershort=" + gendershort + "&nationality=" + user.nation + "&lang=" + $rootScope.currentLang + "&passport=" + user.passport;
			
			$http
			.get(url)
			.success(function(data) {
				if (data.success == true) {
				    msgDialogSrvc.showDialog('#wrap', 'success', data.msg);
				    $state.go(startpage);
				}
				else {
				    msgDialogSrvc.showDialog('#wrap', 'error', data.msg);
				}
			})
		    .error(function (data) {
		            msgDialogSrvc.showDialog('#wrap', 'error', 'request_failed');
		    });
		}
	}
})();