(function() {
	...
	function changepwCtrl($rootScope, $state, $scope, $translate, localStorageService, $http, $stateParams, loginData, $filter, msgDialogSrvc, $mdMedia) {
		...
		$scope.getChangePassword = function (user) {
		    ...
		    if (!user || !user.pw1 || user.pw1.length < 6) {
		        msgDialogSrvc.showDialog('#registerForm', 'error', 'min_password_length');
		        return;
		    }

		    if (user.pw1 != user.pw2) {
		        msgDialogSrvc.showDialog('#registerForm', 'error', 'passwords_are_not_equal');
		        return;
		    }
